...
public class GlobalExceptionsHandler {
    ...
    @ExceptionHandler
    public ResponseEntity<MessagingServiceAppError> handleConstraintValidationException(ConstraintViolationException e) {
        log.error(e.getMessage(), e);
        return new ResponseEntity<>(new MessagingServiceAppError(MessagingServiceAppError.MessagingServiceErrors.MESSAGING_SERVICE_FIELD_VALIDATION.name(), e.getMessage()), HttpStatus.BAD_REQUEST);
    }

    @ExceptionHandler
    public ResponseEntity<MessagingServiceAppError> handleMethodArgumentNotValidException(MethodArgumentNotValidException e) {
        log.error(e.getMessage(), e);
        return new ResponseEntity<>(new MessagingServiceAppError(MessagingServiceAppError.MessagingServiceErrors.MESSAGING_SERVICE_FIELD_VALIDATION.name(), e.getMessage()), HttpStatus.BAD_REQUEST);
    }

    @ExceptionHandler
    public ResponseEntity<MessagingServiceAppError> handleHttpRequestMethodNotSupportedException(HttpRequestMethodNotSupportedException e) {
        log.error(e.getMessage(), e);
        return new ResponseEntity<>(new MessagingServiceAppError(MessagingServiceAppError.MessagingServiceErrors.MESSAGING_SERVICE_BAD_REQUEST.name(), e.getMessage()), HttpStatus.BAD_REQUEST);
    }

    @ExceptionHandler
    public ResponseEntity<MessagingServiceAppError> handleMissingRequestValueException(MissingRequestValueException e) {
        log.error(e.getMessage(), e);
        return new ResponseEntity<>(new MessagingServiceAppError(MessagingServiceAppError.MessagingServiceErrors.MESSAGING_SERVICE_BAD_REQUEST.name(), e.getMessage()), HttpStatus.BAD_REQUEST);
    }

    @ExceptionHandler
    public ResponseEntity<MessagingServiceAppError> handleHttpMessageNotReadableException(HttpMessageNotReadableException e) {
        log.error(e.getMessage(), e);
        return new ResponseEntity<>(new MessagingServiceAppError(MessagingServiceAppError.MessagingServiceErrors.MESSAGING_SERVICE_BAD_REQUEST.name(), e.getMessage()), HttpStatus.BAD_REQUEST);
    }
