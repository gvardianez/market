...
public class TelegramBot extends TelegramLongPollingBot {
    ...
    public void onUpdateReceived(Update update) {
        if (update.hasMessage() && update.getMessage().hasText()) {
            ...
            try {
                if (messageText.startsWith(Command.SEND_IN_CHANNEL.getCommand()) && botConfig.getOwnerId().equals(update.getMessage().getChatId())) {
                    String textToSend = messageText.substring(Command.SEND_IN_CHANNEL.getCommand().length());
                    sendMessageInChannel(textToSend);
                } else {
                    prepareAndSendMessage(update.getMessage().getChatId(), "Недостаточно прав доступа, или команда не поддерживается");
                }
