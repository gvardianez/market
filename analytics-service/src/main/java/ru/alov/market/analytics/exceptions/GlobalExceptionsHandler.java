...
public class GlobalExceptionsHandler {
    ...
    @ExceptionHandler
    public ResponseEntity<AnalyticsServiceAppError> handleConstraintValidationException(ConstraintViolationException e) {
        log.error(e.getMessage(), e);
        return new ResponseEntity<>(new AnalyticsServiceAppError(AnalyticsServiceAppError.AnalyticsServiceErrors.ANALYTICS_SERVICE_FIELD_VALIDATION.name(), e.getMessage()), HttpStatus.BAD_REQUEST);
    }

    @ExceptionHandler
    public ResponseEntity<AnalyticsServiceAppError> handleMethodArgumentNotValidException(MethodArgumentNotValidException e) {
        log.error(e.getMessage(), e);
        return new ResponseEntity<>(new AnalyticsServiceAppError(AnalyticsServiceAppError.AnalyticsServiceErrors.ANALYTICS_SERVICE_FIELD_VALIDATION.name(), e.getMessage()), HttpStatus.BAD_REQUEST);
    }

    @ExceptionHandler
    public ResponseEntity<AnalyticsServiceAppError> handleHttpRequestMethodNotSupportedException(HttpRequestMethodNotSupportedException e) {
        log.error(e.getMessage(), e);
        return new ResponseEntity<>(new AnalyticsServiceAppError(AnalyticsServiceAppError.AnalyticsServiceErrors.ANALYTICS_SERVICE_BAD_REQUEST.name(), e.getMessage()), HttpStatus.BAD_REQUEST);
    }

    @ExceptionHandler
    public ResponseEntity<AnalyticsServiceAppError> handleHttpMessageNotReadableException(HttpMessageNotReadableException e) {
        log.error(e.getMessage(), e);
        return new ResponseEntity<>(new AnalyticsServiceAppError(AnalyticsServiceAppError.AnalyticsServiceErrors.ANALYTICS_SERVICE_BAD_REQUEST.name(), e.getMessage()), HttpStatus.BAD_REQUEST);
    }

    @ExceptionHandler
    public ResponseEntity<AnalyticsServiceAppError> handleMissingRequestValueException(MissingRequestValueException e) {
        log.error(e.getMessage(), e);
        return new ResponseEntity<>(new AnalyticsServiceAppError(AnalyticsServiceAppError.AnalyticsServiceErrors.ANALYTICS_SERVICE_BAD_REQUEST.name(), e.getMessage()), HttpStatus.BAD_REQUEST);
    }
    ...
    public ResponseEntity<AnalyticsServiceAppError> catchAnotherException(Exception e) {
        log.error(e.getMessage(), e);
        return new ResponseEntity<>(new AnalyticsServiceAppError(AnalyticsServiceAppError.AnalyticsServiceErrors.ANALYTICS_SERVICE_INTERNAL_EXCEPTION.name(), HttpStatus.INTERNAL_SERVER_ERROR.name()), HttpStatus.INTERNAL_SERVER_ERROR);
    }
