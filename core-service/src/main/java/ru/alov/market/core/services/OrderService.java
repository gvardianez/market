...
public class OrderService {
    ...
    @Transactional
    public Mono<Order> createNewOrder(@NotBlank String username, @NotNull @Email String email, @Valid OrderDetailsDto orderDetailsDto) {
        ...
        return cartServiceIntegration.getCurrentUserCart(username).flatMap(cartDto -> {
                    ...
                    return promotionServiceIntegration.getRecalculateCart(new RecalculateCartRequestDto(cartDto, LocalDateTime.now())).map(
                            cartDto1 -> {
    ...
    public List<Order> findOrdersByProductIdAndUsernameAndOrderStatus(@NotBlank String username, @NotNull Long productId, @NotBlank String status) {
